...
class Model(nn.Module):
	...
	def combine_dual_views(self, x1, x2, users, items):
		T = args.temp
		user_embeddings1, item_embeddings1 = torch.split(x1, [args.user, args.item], dim=0)
		user_embeddings2, item_embeddings2 = torch.split(x2, [args.user, args.item], dim=0)
		...
		all_users  =  torch.mean(torch.stack([user_embs1, user_embs2],dim=0), dim=0)
		return all_users

	def combine_single_views(self, x1, x2, users, items):
		T = args.temp
		user_embeddings1, item_embeddings1 = torch.split(x1, [args.user, args.item], dim=0)
		...
		return user_embs1
	def split_user_embedding(self,x1,index):
		user_embeddings = F.normalize(x1, dim=1)
		user_embs = F.embedding(index, user_embeddings)
		return  user_embs
...
class DenoisingNet(nn.Module):
	...
	def hard_concrete_sample(self, log_alpha, beta=1.0, training=True): # 生成一个基于硬 concrete 分布的样本
		gamma = args.gamma
		zeta = args.zeta
